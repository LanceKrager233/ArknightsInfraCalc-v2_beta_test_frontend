...
export function DataConsentDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next && !saving) onDecline(); }}>
      <DialogContent data-cloud-consent-dialog className="sm:max-w-lg">
        <DialogHeader>
        ...
        <DialogFooter>
          <Button type="button" variant="outline" disabled={saving} onClick={onDecline}>继续纯本地模式</Button>
          <Button type="button" disabled={saving || !terms || !privacy} onClick={onAccept}>{saving ? "正在启用…" : "同意并开始同步"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
