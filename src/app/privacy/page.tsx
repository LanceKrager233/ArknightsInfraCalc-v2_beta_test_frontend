  ...
  return (
    <LegalDocument eyebrow="明日方舟基建排班助手" title="隐私政策" effectiveDate={LEGAL_EFFECTIVE_DATE}>
      ...
      <section>
        <h2>3. 处理方式与目的</h2>
        <ul>
          <li>登录凭证经 AES-256-GCM 加密后存入此浏览器的 HttpOnly Cookie，请求期间由本站服务端解密使用，不写入业务数据库。</li>
      ...
      <section>
        <h2>4. 保存期限</h2>
        <ul>
          <li>森空岛登录凭证自扫码成功起固定保存 7 天，刷新页面或 token 不会延长期限。</li>
          <li>服务端 CLI 运行记录最多保存 7 天，你也可以随时提前删除。</li>
          <li>浏览器中的布局、干员 Box 和最近排班通常最多保存 30 天；“删除全部森空岛数据”会立即移除其中的森空岛来源内容。</li>
          <li>未完成的二维码登录记录最多保留 10 分钟。</li>
        </ul>
      </section>
